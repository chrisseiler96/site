...
<section class="bentobox">
  <h2 class="
    ...
    md:text-7xl
  ">
    {#if heading.includes('Gold Standard')}
      {@html heading.replace('Gold Standard', '<em class="bg-gradient-to-b from-yellow-100 to-yellow-500 bg-clip-text not-italic text-transparent">Gold Standard</em>')}
    {:else}
      {heading}
    {/if}
  </h2>
  ...
  <div class="mt-16 grid max-w-4xl grid-rows-[auto_auto_auto] gap-8 md:grid-cols-3 md:gap-10">
    {#each boxes as box}
      <div 
        ...
        class:md:col-span-2={box.wide}
        class:md:col-span-1={!box.wide}
      >
        <h3 class="text-2xl">{box.title}</h3>
        <div class="max-w-md text-balance text-gray-300">
          <p>{box.body}</p>
        </div>
        {#if box.imageSrc}
          <img src={box.imageSrc} alt={box.title} class="max-h-36 w-auto"/>
        {/if}
      </div>
    {/each}
  </div>
